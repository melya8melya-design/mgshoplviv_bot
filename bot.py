...
def get_stock_with_folders():
    try:
        # Залишки (містить собівартість, не ціну продажу)
        ...
        # Будуємо словник залишків по id товару
        stock_map = {}
        product_ids = []
        ...
        # Ціни продажу — беремо через assortment (повертає salePrices)
        # Робимо запити батчами по 100
        price_map = {}
        batch_size = 100
        for i in range(0, len(product_ids), batch_size):
            batch = product_ids[i:i+batch_size]
            filter_str = ";".join([f"id={pid}" for pid in batch])
            try:
                p_resp = requests.get(
                    f"{MS_BASE}/entity/assortment?filter={filter_str}&limit={batch_size}",
                    headers=ms_headers(), timeout=15
                )
                p_resp.raise_for_status()
                for prod in p_resp.json().get("rows", []):
                    pid = prod.get("id", "")
                    sale_prices = prod.get("salePrices", [])
                    # Перша ціна продажу (основна)
                    price = 0
                    if sale_prices:
        ...
        # Збираємо каталог
        catalog = {}
        for pid, stock_info in stock_map.items():
            folder_href = stock_info["folder_href"]
...
def products_keyboard(products, folder_idx):
    ...
    for i, p in enumerate(products):
        ...
        buttons.append([InlineKeyboardButton(
            f"{name} | {p['price']:.0f}грн | {p['stock']}ящ",
