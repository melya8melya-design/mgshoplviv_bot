...
def get_stock_with_folders():
    try:
        ...
        # Ціни продажу через /entity/product з expand=salePrices
        # Беремо всі товари одним запитом
        products_resp = requests.get(
            f"{MS_BASE}/entity/product?limit=1000&fields=id,salePrices",
            headers=ms_headers(), timeout=20
        )
        products_resp.raise_for_status()
        price_map = {}
        for prod in products_resp.json().get("rows", []):
            pid = prod.get("id", "")
